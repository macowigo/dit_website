...
class RevSliderFront extends RevSliderFunctions {
	...
	public static function add_admin_bar(){
		...
		<script type="text/javascript">
			function rs_adminBarToolBarTopFunction() {
				if(jQuery('#admin-bar-revslider-default').length > 0 && jQuery('rs-module-wrap').length > 0){
					...
					if(aliases.length > 0){
						jQuery('#admin-bar-revslider-default li').each(function(){
							var li = jQuery(this),
					...
					jQuery('#admin-bar-revslider').remove();
				}
			}
	...
	public static function add_admin_menu_nodes(){
		...
		self::_add_node('<span class="rs-label">Slider Revolution</span>', false, admin_url('admin.php?page=revslider'), array('class' => 'revslider-menu'), 'revslider'); //<span class="menu-image dashicons-before dashicons-update"></span>
		...
		if(!empty($sliders)){
			foreach ($sliders as $id => $slider){
				self::_add_node('<span class="rs-label" data-alias="' . esc_attr($slider['alias']) . '">' . esc_html($slider['title']) . '</span>', 'revslider', admin_url('admin.php?page=revslider&view=slide&id=slider-'.$id), array('class' => 'revslider-sub-menu'), esc_attr($slider['alias'])); //<span class="menu-image dashicons-before dashicons-update"></span>
			}
		}
	}
