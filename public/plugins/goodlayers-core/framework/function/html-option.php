		...
		class gdlr_core_html_option{
			...
			static function include_script($elements = array()){
				...
				wp_enqueue_script('gdlr-core-html-option', GDLR_CORE_URL . '/framework/js/html-option.js', array(
					'jquery', 'jquery-effects-core', 'color-picker', 'jquery-ui-slider', 'jquery-ui-datepicker'
				), '1.4.5', true);	
...
<?php
			...
			static function tmce_init(){
				...
				$pb_tmce  = '<div id="' . esc_attr($editor_id) . '-wrap" class="core-ui editor-wrap ' . esc_attr($set['switch_class']) . '">';
				$pb_tmce .= empty($set['editor_css'])? '': $set['editor_css']; 
				if( !empty($set['switch_button']) || !empty($set['media_buttons']) ){
					$pb_tmce .= '<div id="' . esc_attr($editor_id) . '-editor-tools" class="editor-tools hide-if-no-js">';
					if( !empty($set['media_buttons']) ){
						$pb_tmce .= '<div id="' . esc_attr($editor_id) . '-media-buttons" class="media-buttons">';
						
						if( !function_exists('media_buttons') ){
							include(ABSPATH . 'admin/includes/media.php');
						}
						...
						$pb_tmce .= '</div>'; // media-buttons
					}
					$pb_tmce .= '<div class="editor-tabs">';
					if( $set['switch_button'] ){
						$pb_tmce .= '<button type="button" id="' . esc_attr($editor_id) . '-tmce" class="switch-editor switch-tmce" data-editor-id="' . esc_attr($editor_id) . '">Visual</button>';
						$pb_tmce .= '<button type="button" id="' . esc_attr($editor_id) . '-html" class="switch-editor switch-html" data-editor-id="' . esc_attr($editor_id) . '">Text</button>';
					}
					$pb_tmce .= '</div>'; // editor-tabs
					$pb_tmce .= '</div>'; // editor-tools
				}
				
				// content editor area
				$pb_tmce_content  = '<div id="' . esc_attr($editor_id) . '-editor-container" class="editor-container">';
				...
				$pb_tmce_content .= '</div>'; // editor-container
