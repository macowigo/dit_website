		...
		$('.gdlr-core-page-option-content').each(function(){
			...
			$(this).siblings('.gdlr-core-page-option-head').on('click', '.gdlr-core-page-option-head-save', function(){
				...
				$.ajax({
					...
					data: { 'security': security, 'action': 'gdlr_core_save_page_option_data', 'post_id': post_id, 'name': data_input.attr('name'), 'value': data_input.val() },
					dataType: 'json',
...
(function($){
	...
	function gdlr_core_init_tmce_element( obj ){
		...
		if( typeof tinymce !== 'undefined' ){
			...
			var tmce_wrap = tinymce.$('#' + tmce_id + '-wrap'); 
	...
	function gdlr_core_get_tmce_data( obj ){
		...
		if( typeof(tinymce) != 'undefined' ){
			if(tinymce.$('#' + tmce_id + '-wrap').hasClass('tmce-active')){
				return tinymce.get(tmce_id).getContent();
			}else if(tinymce.$('#' + tmce_id + '-wrap').hasClass('html-active')){
	...
	function gdlr_core_wp_Nop( html ) {
		...
		if ( html.indexOf( '[caption' ) !== -1 ) {
			...
			html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
				return a.replace( /<br([^>]*)>/g, '<temp-br$1>' ).replace( /[\r\n\t]+/, '' );
			});
		}
		...
		if ( preserve_linebreaks ) {
			html = html.replace( /<line-break>/g, '\n' );
		}

		if ( preserve_br ) {
			html = html.replace( /<temp-br([^>]*)>/g, '<br$1>' );
		}

		// Restore preserved tags.
		if ( preserve.length ) {
			html = html.replace( /<preserve>/g, function() {
