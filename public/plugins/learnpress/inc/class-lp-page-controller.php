...
class LP_Page_Controller {
	...
	public function set_title_pages( $title ) {
		...
		if ( ! empty( $course_archive_page_id ) && $wp_query->post &&
			...
			if ( is_plugin_active( 'wordpress-seo/seo.php' ) || is_plugin_active( 'wordpress-seo-premium/seo-premium.php' ) ) {
	...
	protected function _is_profile() {
		...
		if ( empty( $>query_vars['user'] ) ) {
			...
			if ( $wp_user = get_user_by( 'login', urldecode( $>query_vars['user'] ) ) ) {
		...
		try {
			if ( ! $viewing_user ) {
				throw new Exception( sprintf( '%s %s %s', __( 'The user', 'learnpress' ), $>query_vars['user'], __( 'is not available!', 'learnpress' ) ) );
			}
		} catch ( Exception $ex ) {
...
//				$wp_query->is_category = learn_press_is_course_category();
				// Remove causing an error error :: $ taxonomy when yoast seo active
