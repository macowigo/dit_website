...
class RWMB_Color_Field extends RWMB_Text_Field {
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'rwmb-color', RWMB_CSS_URL . 'color.css', array( 'color-picker' ), RWMB_VER );

		$dependencies = array( 'color-picker' );
		$args         = func_get_args();
		$field        = reset( $args );
		if ( ! empty( $field['alpha_channel'] ) ) {
			wp_enqueue_script( 'color-picker-alpha', RWMB_JS_URL . 'color-picker-alpha/color-picker-alpha.min.js', array( 'color-picker' ), RWMB_VER, true );
			$dependencies = array( 'color-picker-alpha' );
		}
