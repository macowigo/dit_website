 ...
 */
class LPR_Export_Import_Parser {
	function parse( $file ) {
 ...
 */
class LPR_Export_Import_Parser_SimpleXML {
	function parse( $file ) {
...
//			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'wordpress-importer' ) );
		...
		foreach ( $xml->channel->item as $item ) {
			...
			$post['post_date']      = (string) $>post_date;
			$post['post_date_gmt']  = (string) $>post_date_gmt;
			$post['comment_status'] = (string) $>comment_status;
			$post['ping_status']    = (string) $>ping_status;
			$post['post_name']      = (string) $>post_name;
			$post['status']         = (string) $>status;
			$post['post_parent']    = (int) $>post_parent;
			$post['menu_order']     = (int) $>menu_order;
			$post['post_type']      = (string) $>post_type;
			$post['post_password']  = (string) $>post_password;
			$post['is_sticky']      = (int) $>is_sticky;
			$post['post_author_id'] = (int) $>post_author_id;
			$post['parser']         = __CLASS__;
			...
			if ( $post['post_type'] == 'lp_course' ) {
				...
				if ( isset( $>section ) ) {
					foreach ( $>section as $_section ) {
						$section = array(
							'section_course_id'   => (int) $_section->section_course_id,
						...
						if ( isset( $_section->section_item ) ) {
							foreach ( $_section->section_item as $_item ) {
								$section['items'][] = array(
									'item_id'    => (int) $_item->item_id,
									'item_order' => (int) $_item->item_order,
									'item_type'  => (string) $_item->item_type,
								);
							}
						}
		...
		$plugin_name = $xml->xpath( '/rss/channel/wp:plugin_name' );
		$plugin_name = (string) trim( $plugin_name[0] );

		$plugin_version = $xml->xpath( '/rss/channel/wp:plugin_version' );
		$plugin_version = (string) trim( $plugin_version[0] );
	...
	function tag_close( $parser, $tag ) {
		switch ( $tag ) {
			...
			case 'wp:wxr_version':
				...
				break;
			case 'wp:plugin_name':
				$this->plugin_name = $this->cdata;
				break;
			case 'wp:plugin_version':
				$this->plugin_version = $this->cdata;
				break;
			default:
				if ( $this->in_sub_tag ) {
	...
	function parse( $file ) {
		...
		if ( $fp ) {
			while ( !$this->feof( $fp ) ) {
				...
				if ( false !== strpos( $importline, '<wp:plugin_name>' ) ) {
					preg_match( '|<wp:plugin_name>(.*?)</wp:plugin_name>|is', $importline, $plugin_name );
					$this->plugin_name = $plugin_name[1];
					continue;
				}
				if ( false !== strpos( $importline, '<wp:plugin_version>' ) ) {
					preg_match( '|<wp:plugin_version>(.*?)</wp:plugin_version>|is', $importline, $plugin_version );
					$this->plugin_version = $plugin_version[1];
					continue;
				}
...
//			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'wordpress-importer' ) );
	...
	function process_post( $post ) {
		...
		$post_author_id = $this->get_tag( $post, 'wp:post_author_id' );
		...
		if ( $attachment ) {
			preg_match_all( '|<wp:attachment id="([^"]+?)" mime_type="([^"]+?)" filename="([^"]+?)">(.+?)</wp:attachment>|is', $post, $attachments, PREG_SET_ORDER );
			if ( $attachments ) {
				$postdata['attachment'] = array(
					'id'        => $attachments[0][1],
		...
		if ( $post_type == 'lp_course' ) {
			...
			if ( $sections ) {
				foreach ( $sections as $section ) {
					$_section = array(
						'section_name'        => $this->get_tag( $section, 'wp:section_name' ),
		...
		if ( $post_type == 'lp_quiz' ) {
			///
			$postdata['questions'] = array();
			preg_match_all( '|<wp:question>(.+?)</wp:question>|is', $post, $questions );
		...
		if ( $post_type == 'lp_question' ) {
			///
			$postdata['answers'] = array();
			if ( preg_match_all( '|<wp:answer>(.+?)</wp:answer>|is', $post, $answers ) ) {
				$answers = $answers[1];
				if ( $answers ) {
					foreach ( $answers as $answer ) {
						$_answer = array(
							'question_id'  => $this->get_tag( $answer, 'wp:question_id' ),
							'answer_data'  => $this->get_tag( $answer, 'wp:answer_data' ),
							'answer_order' => $this->get_tag( $answer, 'wp:answer_order' ),
						);

						$postdata['answers'][] = $_answer;
					}
				}
			}
		}
