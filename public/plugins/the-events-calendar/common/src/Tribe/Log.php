...
class Tribe__Log {
	 ...
	 *
	 * This will only apply to CLI based logging.
	 *
	 * @param string $entry
	 * @param string $src
	 */
	...
	/**
	 * Adds an entry to the log (if it is at the appropriate level, etc) and outputs information using CLI if available.
	 *
	...
	public function log( $entry, $type = self::DEBUG, $src = '' ) {
		...
		/**
		 * Whether to log the message to cli, if available, or not.
		 *
		 * @since 4.9.6
		 *
		 * @param bool $log_to_wpcli Whether to log to cli, if available, or not.
		...
		// Only go further if we have WP_CLI or if we want to log to CLI.
		if ( ! class_exists( 'WP_CLI' ) || false === $log_to_wpcli ) {
			return false;
		}

		// We are always logging to CLI if available
