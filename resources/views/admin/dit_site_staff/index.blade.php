  ...
  <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">{{ Breadcrumbs::render('admin.staff.index') }}</a></li>
        </ol>
      </div><!-- /.col -->
  </div><!-- /.row -->
@endsection
...
<div class="modal fade" id="addstaffDetailsModal{{$uniqueID}}" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="addstaffDetailsModalTitle{{$uniqueID}}Label">{{ trans('staff.create') }}</h4>
        ...
        <div class="modal-body">
          <div id="addstaffDetailsModalBody{{$uniqueID}}"></div>
        </div>
...
function add_staff_Details_Modal{{$uniqueID}}() {
     $('#addstaffDetailsModalBody{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
    let request = $.get("{{ route('admin.staff.create') }}");
      request.done(function (e) {
        $('#addstaffDetailsModalBody{{$uniqueID}}').html(e);
    })
    request.fail(function(jqXHR, exception){
      ...
      $('#adstaffDetailsModalBody{{$uniqueID}}').html(msg);
    });//fail
...
<div class="col-md-12">
    <div class="card card-default">
        ...
        <div class="card-header">
          <h4  class="my-1 float-left"> <i class="fas fa-info-circle blue"></i>&nbsp;{{ trans('staff.model_plural') }}</h4>

          <div class="btn-group btn-group-sm float-right" role="group">

               <a href="#"
                       class="btn btn-success"
                       data-target="#addstaffDetailsModal{{$uniqueID}}"
                       data-toggle="modal"
                       data-whatever="@mdo"
                       onclick="add_staff_Details_Modal{{$uniqueID}}()"
                       title=" title="{{ trans('staff.create') }}"">
        ...
        @if($staffs == 0)
            <div class="card-body text-center">
                <h4>{{ trans('staff.none_available') }}</h4>
            </div>
        @else
        <div class="card-body">
            <div class="table-responsive">
                ...
                <thead>
                    <tr>
                            <th>{{ trans('staff.prefix') }}</th>
                            <th>{{ trans('staff.fname') }}</th>
                            <!-- <th>{{ trans('staff.mname') }}</th> -->
                            <th>{{ trans('staff.lname') }}</th>
                            <!-- <th>{{ trans('staff.imgurl') }}</th> -->
                            <th>{{ trans('staff.email') }}</th>
                            <!-- <th>{{ trans('staff.phone') }}</th>
                            <th>{{ trans('staff.designation') }}</th>
                            <th>{{ trans('staff.title') }}</th> -->
