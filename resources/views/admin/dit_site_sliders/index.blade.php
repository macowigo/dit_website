  ...
  <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">{{ Breadcrumbs::render('admin.slider.index') }}</a></li>
        </ol>
      </div><!-- /.col -->
  </div><!-- /.row -->
@endsection
...
<div class="modal fade" id="addSliderDetailsModal{{$uniqueID}}" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="addSliderDetailsModalTitle{{$uniqueID}}Label">{{ trans('sliders.create') }}</h4>
        ...
        <div class="modal-body">
          <div id="addSliderDetailsModalBody{{$uniqueID}}"></div>
        </div>
...
function add_slider_details_modal{{$uniqueID}}() {
     $('#addSliderDetailsModalBody{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
    let request = $.get("{{ route('admin.slider.create') }}");
      request.done(function (e) {
        $('#addSliderDetailsModalBody{{$uniqueID}}').html(e);
    })
    request.fail(function(jqXHR, exception){
      ...
      $('#addquickLinkDetailsModalBody{{$uniqueID}}').html(msg);
    });//fail
...
<div class="col-md-12">
    <div class="card card-default">
        ...
        <div class="card-header">
          <h4  class="my-1 float-left"> <i class="fas fa-info-circle blue"></i>&nbsp;{{ trans('sliders.model_plural') }}</h4>

          <div class="btn-group btn-group-sm float-right" role="group">
               <a href="#"
                       class="btn btn-success"
                       data-target="#addSliderDetailsModal{{$uniqueID}}"
                       data-toggle="modal"
                       data-whatever="@mdo"
                       onclick="add_slider_details_modal{{$uniqueID}}()"
                       title=" title="{{ trans('sliders.create') }}"">
            ...
            <div class="table-responsive">
                ...
                <thead>
                    <tr>
                            ...
                            <th>{{ trans('sliders.caption') }}</th>
                            <th>{{ trans('sliders.url') }}</th>
                            <th>{{ trans('sliders.is_public') }}</th>
