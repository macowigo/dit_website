...
<div class="card">
    ...
    <div class="card-body">
        <form  accept-charset="UTF-8" id="edit_slider_form{{$uniqueID}}" name="edit_slider_form" class="form-horizontal" enctype="multipart/form-data">
        {{ csrf_field() }}
        @method('PUT')
...
if ($("#edit_slider_form{{$uniqueID}}").length > 0) {

 $("#edit_slider_form{{$uniqueID}}").validate({
   rules: {
 ...
 submitHandler: function(form) {
  $('#viewLoadingStatus{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
   $.ajax({
       ...
       type: "POST",
       data:new FormData(document.getElementById('edit_slider_form{{$uniqueID}}')),
       dataType:'JSON',
       contentType: false,
       cache: false,
       processData: false,
