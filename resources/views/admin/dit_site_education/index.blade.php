  ...
  <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">{{ Breadcrumbs::render('admin.education.index') }}</a></li>
        </ol>
      </div><!-- /.col -->
  </div><!-- /.row -->
@endsection
...
?>
<div class="modal fade" id="addeducationDetailsModal{{$uniqueID}}" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="addeducationDetailsModalTitle{{$uniqueID}}Label">{{ trans('education.create') }}</h4>
        ...
        <div class="modal-body">
          <div id="addeducationDetailsModalBody{{$uniqueID}}"></div>
        </div>
...
function add_education_Details_Modal{{$uniqueID}}() {
     $('#addeducationDetailsModalBody{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
    let request = $.get("{{ route('admin.education.create') }}");
      request.done(function (e) {
        $('#addeducationDetailsModalBody{{$uniqueID}}').html(e);
    })
    request.fail(function(jqXHR, exception){
      ...
      $('#adeducationDetailsModalBody{{$uniqueID}}').html(msg);
    });//fail
...
<div class="col-md-12">
    <div class="card card-default">
        ...
        <div class="card-header">
          <h4  class="my-1 float-left"> <i class="fas fa-info-circle blue"></i>&nbsp;{{ trans('education.model_plural') }}</h4>

          <div class="btn-group btn-group-sm float-right" role="group">

               <a href="#"
                       class="btn btn-success"
                       data-target="#addeducationDetailsModal{{$uniqueID}}"
                       data-toggle="modal"
                       data-whatever="@mdo"
                       onclick="add_education_Details_Modal{{$uniqueID}}()"
                       title=" title="{{ trans('education.create') }}"">
        ...
        @if($education == 0)
            <div class="card-body text-center">
                <h4>{{ trans('education.none_available') }}</h4>
            </div>
        @else
        <div class="card-body">
            <div class="table-responsive">
                ...
                <thead>
                    <tr>
                            <th>{{ trans('education.staff_id') }}</th>
...
var _datatable = $('#datatable{{$uniqueID}}').DataTable({
    ...
    ajax: {
      url:'{!! route('admin.education.data') !!}'
    },
    columns: [
