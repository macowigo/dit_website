...
<div class="card">
    ...
    <div class="card-body">
        <form  accept-charset="UTF-8" id="edit_event_form{{$uniqueID}}" name="edit_event_form" class="form-horizontal" enctype="multipart/form-data">
        @method('PUT')
        @include ('admin.dit_site_events.form', [
            ...
            <div class="form-group">
                <div class="col-md-offset-2 col-md-10">
                    <input class="btn btn-primary" type="submit" value="{{ trans('events.update') }}">
                </div>
            </div>

        </form>
        <div id="viewResponseDetails{{$uniqueID}}"></div>
    </div>
</div>
...
if ($("#edit_event_form{{$uniqueID}}").length > 0) {

 $("#edit_event_form{{$uniqueID}}").validate({
   rules: {
 ...
 submitHandler: function(form) {
  $('#viewLoadingStatus{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
   $.ajax({
       ...
       type: "POST",
       data:new FormData(document.getElementById('edit_event_form{{$uniqueID}}')),
       dataType:'JSON',
       contentType: false,
       cache: false,
       processData: false,
