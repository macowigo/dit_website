...
<div class="card-body">
 <div id="viewLoadingStatus{{$uniqueID}}"></div>
    <form method="POST" action="{{ route('admin.user.update_role', $user->id) }}" id="edit_user_role_form{{$uniqueID}}" name="edit_user_form{{$uniqueID}}" accept-charset="UTF-8" class="form-horizontal">
    {{ csrf_field() }}
       <input name="_method" type="hidden" value="PUT">
        ...
        <div id="viewResponseAfterEditingUserRoleDetails"></div>
  </div>
...
if ($("#edit_user_role_form{{$uniqueID}}").length > 0) {

 $("#edit_user_role_form{{$uniqueID}}").validate({
   rules: {
         name: {
             required: true,
             minlength: 5,
 ...
 submitHandler: function(form) {
  $('#viewLoadingStatus{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
   $.ajax({
       url: "{{route('admin.user.update_role',$user->id)}}",
       type: "PUT",
       data: $("#edit_user_role_form{{$uniqueID}}").serialize(),
       success: function(response) {
          ...
          if(!response.error){
            ...
            $('#viewLoadingStatus{{$uniqueID}}').html("");
            sleep(2000).then(() => {
              $('#viewResponseAfterEditingUserRoleDetails{{$uniqueID}}').html(msgs);
