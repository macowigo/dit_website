...
<div class="card">
    ...
    <div class="card-body">
        <form  accept-charset="UTF-8" id="create_quick_link_form{{$uniqueID}}" name="create_quick_link_form" class="form-horizontal" enctype="multipart/form-data">
        {{ csrf_field() }}
        @include ('admin.quick_links.form', [
            ...
            <div class="form-group">
                <div class="col-md-offset-2 col-md-10">
                    <input class="btn btn-primary" type="submit" value="{{ trans('quick_links.add') }}">
                </div>
            </div>
...
if ($("#create_quick_link_form{{$uniqueID}}").length > 0) {

 $("#create_quick_link_form{{$uniqueID}}").validate({
   rules: {
 ...
 submitHandler: function(form) {
  $('#viewLoadingStatus{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
   $.ajax({
       url: "{{ route('admin.quick_link.store') }}",
       type: "POST",
       data: $("#create_quick_link_form{{$uniqueID}}").serialize(),
