...
<div class="card">
    ...
    <div class="card-body">
        <form  accept-charset="UTF-8" id="create_skills_form{{$uniqueID}}" name="create_skills_form" class="form-horizontal" enctype="multipart/form-data">
        {{ csrf_field() }}
        @include ('admin.dit_site_skill.form', [
            ...
            <div class="form-group">
                <div class="col-md-offset-2 col-md-10">
                    <input class="btn btn-primary" type="submit" value="{{ trans('skills.add') }}">
                </div>
            </div>
...
if ($("#create_skills_form{{$uniqueID}}").length > 0) {

 $("#create_skills_form{{$uniqueID}}").validate({
   rules: {
 ...
 submitHandler: function(form) {
  $('#viewLoadingStatus{{$uniqueID}}').html("<div class='loader'>Loading...</div>");
   $.ajax({
       ...
       type: "POST",
       data: $("#create_skills_form{{$uniqueID}}").serialize(),
       success: function(response) {
          ...
          if(!response.error){
            ...
            $('#viewLoadingStatus{{$uniqueID}}').html("");
            $('#viewResponseAfterAddingSkillsDetails{{$uniqueID}}').html(msgs);
            sleep(2000).then(() => {
              $('#viewResponseAfterAddingSkillsDetails{{$uniqueID}}').html("");
            });
          }
       }
    });
